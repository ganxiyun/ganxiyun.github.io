...
More formally, finite automata in the [Automata theory] should be used.
...
Given a text and a Regular Expression, judge whether the text can be denoted by the Regular Expression. For example, use `[\w.]+@\w+\.com` to denote all email addresses. The following figure shows the 4 combinations. It even can not pass a@example.net. Definitely it's not acceptable for validation in user registration. But if you just want to extract some email addresses from huge logs, it's not a bad choice.
...
`*.txt` is used to match all `txt` files in the command line. However, it's easy to misuse it as Regular Expression in programming language. I saw `works.xiaoz.*` was used in Java Regular Expression to match all packages in `works.xiaoz`. 
...
`[\w.]+@\w+\.com` showed above is an incorrect Regular Expression to validate email addresses. The correct one [Mail::RFC822::Address: regexp-based address validation] is unbelievably 6.2 KiB long to validate email address. 
